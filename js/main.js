// Generated by CoffeeScript 1.7.1
(function() {
  $(function() {
    var boardIsFull, generateTile, getColumn, getRandomCellIndecies, getRow, ppArray, randomIndex, randomValue;
    ppArray = function(array) {
      var row, _i, _len, _results;
      _results = [];
      for (_i = 0, _len = array.length; _i < _len; _i++) {
        row = array[_i];
        _results.push(console.log(row));
      }
      return _results;
    };
    this.board = [0, 1, 2, 3].map(function(x) {
      return [0, 1, 2, 3].map(function(y) {
        return 0;
      });
    });
    randomIndex = function(x) {
      return Math.floor(Math.random() * 4);
    };
    getRandomCellIndecies = function() {
      return [randomIndex(4), randomIndex(4)];
    };
    randomValue = function() {
      var val, values;
      values = [2, 2, 2, 2, 2, 4];
      return val = values[randomIndex(values.length)];
    };
    boardIsFull = function(board) {
      var x, y, _i, _j;
      for (x = _i = 0; _i <= 3; x = ++_i) {
        for (y = _j = 0; _j <= 3; y = ++_j) {
          if (board[x][y] === 0) {
            return false;
          }
        }
      }
      return true;
    };
    generateTile = function(board) {
      var val, x, y, _ref;
      if (!boardIsFull(board)) {
        val = randomValue();
        _ref = getRandomCellIndecies(), x = _ref[0], y = _ref[1];
        if (board[x][y] === 0) {
          return board[x][y] = val;
        } else {
          return generateTile(board);
        }
      }
    };
    $('body').keydown(function(e) {
      var key, keys;
      key = e.which;
      keys = [37, 38, 39, 40];
      if ($.inArray(key, keys) > -1) {
        e.preventDefault();
      }
      switch (key) {
        case 37:
          return console.log('left');
        case 38:
          return console.log('up');
        case 39:
          return console.log('right');
        case 40:
          return console.log('down');
      }
    });
    generateTile(this.board);
    generateTile(this.board);
    ppArray(this.board);
    getRow = function(r, board) {
      return board[r];
    };
    console.log(getRow(2, this.board));
    getColumn = function(c, board) {
      var b;
      b = board;
      return [b[0][c], b[1][c], b[2][c], b[3][c]];
    };
    return console.log(getColumn(3, this.board));
  });

}).call(this);

//# sourceMappingURL=main.map
